...
public class ItemServiceTests {
    ...
    public void givenThreeItem_whenGetUserItems_whenReturnThreeItems() {
        ...
        List<ItemPresentDto> items = List.of(ItemPresentDto.builder().build(),
                ...
                ItemPresentDto.builder().build());
        Mockito.when(itemOwnerService.getUserItems(anyInt(), any(Pageable.class))).thenReturn(items);

        //when
        List<ItemPresentDto> obtainedItems = itemService.getUserItems(1, Pageable.unpaged());
    ...
    public void givenTwoItems_whenGetAvailableItemsByDescription_thenReturnTwoItems() {
        ...
        Mockito.when(itemRepository.findByNameOrDescription(anyString(), anyBoolean(), any(Pageable.class))).thenReturn(items);

        //when
        List<ItemDto> itemDtos = itemService.getAvailableItemsByName("dfsdf", Pageable.unpaged());
