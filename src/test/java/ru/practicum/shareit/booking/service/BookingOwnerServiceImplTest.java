...
public class BookingOwnerServiceImplTest {
    ...
    public void givenUserIdAndState_whenFindBookings_thenReturnBooking() {
        ...
        //when
        Mockito.when(bookingOwnerRepository.findAllByItemOwnerIdOrderByStartDesc(anyInt(), any(Pageable.class))).thenReturn(List.of(booking));
        returnedBookings = bookingOwner.findBookingByCondition(userId, StateParam.ALL, Pageable.unpaged());

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingOwnerRepository.findAllByItemOwnerIdAndEndLessThanOrderByStartDesc(anyInt(), any(LocalDateTime.class), any(Pageable.class))).thenReturn(List.of(booking));
        returnedBookings = bookingOwner.findBookingByCondition(userId, StateParam.PAST, Pageable.unpaged());

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingOwnerRepository.findAllByItemOwnerIdAndStartGreaterThanOrderByStartDesc(anyInt(), any(LocalDateTime.class), any(Pageable.class))).thenReturn(List.of(booking));
        returnedBookings = bookingOwner.findBookingByCondition(userId, StateParam.FUTURE, Pageable.unpaged());

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingOwnerRepository.findAllByItemOwnerIdAndStatusOrderByStartDesc(anyInt(), any(Status.class), any(Pageable.class))).thenReturn(List.of(booking));
        returnedBookings = bookingOwner.findBookingByCondition(userId, StateParam.WAITING, Pageable.unpaged());
        ...
        //when
        Mockito.when(bookingOwnerRepository.findAllByItemOwnerIdAndStatusOrderByStartDesc(anyInt(), any(Status.class), any(Pageable.class))).thenReturn(List.of(booking));
        returnedBookings = bookingOwner.findBookingByCondition(userId, StateParam.REJECTED, Pageable.unpaged());

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingOwnerRepository.findCurrentBooking(anyInt(), any(LocalDateTime.class), any(Pageable.class))).thenReturn(List.of(booking));
        returnedBookings = bookingOwner.findBookingByCondition(userId, StateParam.CURRENT, Pageable.unpaged());
