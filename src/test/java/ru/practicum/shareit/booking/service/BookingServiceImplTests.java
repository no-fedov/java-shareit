...
public class BookingServiceImplTests {
    ...
    public void givenUserIdAndState_whenFindBookings_thenReturnBooking() {
        ...
        //when
        Mockito.when(bookingRepository.findByBookerIdOrderByStartDesc(anyInt())).thenReturn(List.of(booking));
        returnedBookings = bookingService.findBookingByCondition(userId, StateParam.ALL);

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingRepository.findByBookerIdAndEndLessThanOrderByStartDesc(anyInt(), any(LocalDateTime.class))).thenReturn(List.of(booking));
        returnedBookings = bookingService.findBookingByCondition(userId, StateParam.PAST);

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingRepository.findByBookerIdAndStartGreaterThanOrderByStartDesc(anyInt(), any(LocalDateTime.class))).thenReturn(List.of(booking));
        returnedBookings = bookingService.findBookingByCondition(userId, StateParam.FUTURE);

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingRepository.findByBookerIdAndStatusOrderByStartDesc(anyInt(), any(Status.class))).thenReturn(List.of(booking));
        returnedBookings = bookingService.findBookingByCondition(userId, StateParam.WAITING);
        ...
        //when
        Mockito.when(bookingRepository.findByBookerIdAndStatusOrderByStartDesc(anyInt(), any(Status.class))).thenReturn(List.of(booking));
        returnedBookings = bookingService.findBookingByCondition(userId, StateParam.REJECTED);

        //then
        assertThat(CollectionUtils.isEmpty(returnedBookings)).isFalse();

        //when
        Mockito.when(bookingRepository.findCurrentBooking(any(LocalDateTime.class), anyInt())).thenReturn(List.of(booking));
        returnedBookings = bookingService.findBookingByCondition(userId, StateParam.CURRENT);
